...
Add the `Full PHPUnit Matrix` label to a pull request in `WordPress/wordpress-develop` to run the same PHP and database combinations as the dispatched run on this branch. The label-triggered workflow uses the same path filters as normal PHPUnit runs.
