...
class Tests_Hooks_BuildUniqueId extends WP_UnitTestCase {
	...
	public function test_string_callback_returns_string() {
	...
	public function test_object_callback_returns_string() {
		$a      = new MockAction();
		$result = _wp_filter_build_unique_id( '', array( $a, 'action' ), 10 );
		$this->assertIsString( $result );
	}

	public function test_static_callback_returns_string() {
		$result = _wp_filter_build_unique_id( '', array( 'MockAction', 'action' ), 10 );
		$this->assertIsString( $result );
	}

	public function test_two_different_objects_produce_different_ids() {
		$a = new MockAction();
		$b = new MockAction();
		$this->assertNotSame(
	...
	public function test_same_object_produces_same_id() {
		$a = new MockAction();
		$this->assertSame(
			_wp_filter_build_unique_id( '', array( $a, 'action' ), 10 ),
			_wp_filter_build_unique_id( '', array( $a, 'action' ), 10 )
	...
	public function test_malformed_array_missing_method_returns_null() {
		$a      = new MockAction();
		$result = _wp_filter_build_unique_id( '', array( $a ), 10 );
		$this->assertNull( $result );
	}

	public function test_malformed_array_non_string_method_returns_null() {
		$a      = new MockAction();
