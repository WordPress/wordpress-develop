 ...
 * @group view-config
 *
 * @covers ::wp_get_entity_view_config
 */
class Tests_View_Config_API extends WP_UnitTestCase {
	...
	public function test_view_list_falls_back_to_generic_all_items_title() {
		$config = wp_get_entity_view_config( 'postType', 'does_not_exist' );
	...
	public function test_view_list_uses_post_type_all_items_label() {
		...
		$config = wp_get_entity_view_config( 'postType', 'view_config_cpt' );
	...
	public function test_filter_receives_data_and_entity() {
		...
		wp_get_entity_view_config( 'custom_kind', 'custom_name' );

		$this->assertInstanceOf( 'WP_View_Config_Data', $received_data );
		$this->assertSame(
			array(
				'kind' => 'custom_kind',
				'name' => 'custom_name',
