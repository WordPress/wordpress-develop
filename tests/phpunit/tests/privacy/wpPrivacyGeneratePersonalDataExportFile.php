...
class Tests_Privacy_WpPrivacyGeneratePersonalDataExportFile extends WP_UnitTestCase {
	 ...
	 *
	 * Used when the expected content has a placeholder, i.e. used to avoid second time differences between the test
	 * and code.
	 *
	 * @param string $actual_content    Content with the actual timestamp.
	 * @param string $expected_content  Expected content that has the timestamp placeholder to be replaced with the
	 *                                  actual timestamp.
	 *
	...
	public function data_contents() {
		return array(
			...
			// Happy path.
			'should contain about and export data groups when single group exist' => array(
				'groups'           => array(
					'user' => array(
						'group_label'       => 'User',
						'group_description' => 'User&#8217;s profile data.',
