 ...
 * @group notes
 *
 * @covers ::wp_notify_note_event
 */
class Tests_Comment_WpNotifyNoteEvent extends WP_UnitTestCase {
	...
	public static function wpSetUpBeforeClass( WP_UnitTest_Factory $factory ) {
		...
		self::$follower    = $factory->user->create_and_get( array( 'role' => 'editor' ) );
		self::$resolver    = $factory->user->create_and_get( array( 'role' => 'editor' ) );
	...
	public function test_resolving_notifies_followers_about_the_event(): void {
		...
		wp_add_note_followers( $root->comment_ID, array( self::$follower->ID ) );
	 ...
	 *
	 * @covers ::wp_new_comment_via_rest_notify_postauthor
	 */
	...
	public function test_the_actor_is_not_emailed_about_their_own_action(): void {
		$root = $this->start_thread( 'Please look at this', self::$commenter->ID );
		wp_add_note_followers( $root->comment_ID, array( self::$resolver->ID ) );
	...
	public function test_a_following_post_author_receives_exactly_one_email(): void {
		$root = $this->start_thread( 'Please look at this', self::$commenter->ID );
		wp_add_note_followers( $root->comment_ID, array( self::$post_author->ID ) );
	...
	public function test_mentioned_users_get_the_mention_email_only(): void {
		...
		$root = $this->start_thread( 'Please look at this', self::$commenter->ID );
		wp_add_note_followers( $root->comment_ID, array( $mentioned->ID ) );
	...
	public function test_followers_without_note_access_are_not_emailed(): void {
		...
		$root = $this->start_thread( 'Please look at this', self::$commenter->ID );
		wp_add_note_followers( $root->comment_ID, array( $subscriber->ID ) );
	...
	/**
	 * @covers ::wp_send_note_event_notification
	 */
	...
	public function test_only_followers_are_offered_the_unfollow_link(): void {
		...
		$this->assertStringContainsString(
			wp_get_note_unfollow_url( (int) $root->comment_ID, self::$follower->ID ),
