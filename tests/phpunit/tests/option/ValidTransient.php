...
class Tests_Option_ValidTransient extends WP_UnitTestCase {
	...
	public function test_valid_transient_with_expired_timeout() {
		...
		$past_time = time() - 1000;
		update_option( '_transient_timeout_' . $transient_name, $past_time );
	 ...
	 * @ticket 37040
	 */
	public function test_valid_transient_with_object_cache() {
		if ( ! wp_using_ext_object_cache() ) {
			$this->markTestSkipped( 'Object cache not available.' );
		}

		$transient_name  = 'valid_transient_with_object_cache';
		$transient_value = 'transient_value';

		set_transient( $transient_name, $transient_value, 60 );
		$this->assertTrue( is_valid_transient( $transient_name ) );
