...
EOF;
	...
	public function test_filter_image_srcset_and_sizes_by_image_attributes() {
		...
		$expected_sources = array_slice(
			array_filter(
				array_map(
					function( $source, $image_meta ) {
					...
					$expected_sources,
					array_fill( 0, count( $expected_sources ), $image_meta )
				)
			),
			2
		);
		...
		$this->assertSame(
			$default_image_without_class_attribute,
		...
		$this->assertSame(
			$default_srcset_string_without_class_attribute,
			wp_get_attachment_image_srcset(
				self::$large_id,
				'full',
				false,
		...
		$this->assertSame(
			$default_sizes_string_without_class_attribute,
			wp_get_attachment_image_sizes(
				self::$large_id,
				'full',
				false,
