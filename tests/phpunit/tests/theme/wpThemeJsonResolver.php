...
class Tests_Theme_wpThemeJsonResolver extends WP_UnitTestCase {
	...
	function test_get_global_styles_from_post_does_not_use_uncached_queries() {
		add_filter( 'query', array( $this, 'filter_db_query' ) );
		$query_count = count( $this->queries );
		for ( $i = 0; $i < 3; $i++ ) {
			WP_Theme_JSON_Resolver::get_global_styles_from_post( wp_get_theme() );
		...
		$user_cpt = WP_Theme_JSON_Resolver::get_global_styles_from_post( wp_get_theme() );
		$this->assertEmpty( $user_cpt );

		$user_cpt = WP_Theme_JSON_Resolver::get_global_styles_from_post( wp_get_theme(), true );
