<?php
/**
 * Tests for notes on the wp_global_styles post type.
 *
 * @package WordPress
 * @subpackage REST API
 */
 ...
 * @group themes
 */
class Tests_REST_Global_Styles_Notes extends WP_Test_REST_TestCase {
	...
	/**
	 * Re-registers the note meta, which the test case unregisters after each
	 * test and which is otherwise only registered on `init`.
	 */
	 ...
	 * @param string $anchor Anchor identifier.
	 */
	public function test_anchor_accepts_arbitrary_identifiers( $anchor ) {
		wp_set_current_user( self::$admin_id );
