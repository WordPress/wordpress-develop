...
class WP_Test_REST_Application_Passwords_Controller extends WP_Test_REST_Controller_Testcase {
	...
	/**
	 * Verifies get_user() resolves a user by ID when they are not a member of the main site.
	 *
	 * @ticket 60029
	 * @group ms-required
	 */
	public function test_create_item_for_user_without_role_on_main_site() {
		...
		$data = $response->get_data();
		if ( isset( $data['code'] ) ) {
			$this->assertNotSame( 'rest_user_invalid_id', $data['code'], 'get_user() must resolve user by ID when user is not a member of the main site.' );
		}
		$this->assertSame( 201, $response->get_status() );
