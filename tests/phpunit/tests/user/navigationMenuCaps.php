...
/**
 * Tests capability handling for the `wp_navigation` post type.
 *
 * @group user
 * @group capabilities
 * @group navigation
 */
class Tests_User_NavigationMenuCaps extends WP_Test_REST_TestCase {
	const NAVIGATION_CAPABILITY_MANAGER_ROLE = 'wp_navigation_menu_manager';
	...
	/**
	 * Returns the expected `wp_navigation` post type capability mapping.
	 *
	 * @return string[] Capability mapping keyed by post type capability name.
	 */
	private static function get_expected_navigation_capability_mapping() {
		return array(
	...
	private static function get_navigation_manager_role_capabilities() {
		...
		foreach ( wp_get_navigation_menu_capability_names() as $capability ) {
	...
	public function test_wp_navigation_post_type_uses_navigation_menu_capabilities() {
		...
		foreach ( self::get_expected_navigation_capability_mapping() as $capability_name => $mapped_capability ) {
			$this->assertSame(
				$mapped_capability,
	 ...
	 * @ticket 29213
	 *
	 * @covers ::wp_maybe_grant_navigation_menus_caps
	 */
	...
	public function test_classic_menu_capabilities_are_not_remapped_to_block_navigation_caps() {
		$navigation_caps = wp_get_navigation_menu_capability_names();
	 ...
	 * @ticket 29213
	 *
	 * @covers WP_REST_Navigation_Fallback_Controller::get_item_permissions_check
	 */
