...
class Tests_User_Capabilities extends WP_UnitTestCase {
	 ...
	 */
	public function test_update_role_unhappy_paths( $role, $display_name ) {
		global $wp_roles;
		...
		$wp_roles->update_role( $role, $display_name );
		$this->flush_roles();
		$this->assertSame(
			$names,
			$wp_roles->get_names(),
			'The roles were updated'
		);
	...
	public function test_update_role_should_change_the_display_name_of_a_role() {
		...
		$wp_roles->update_role( $role_name, $expected_display_name );
		$this->flush_roles();
