...
jQuery( document ).ready( function () {
	...
	QUnit.module( 'dashboard', function( hooks ) {
		...
		QUnit.module( 'onThisDay', function( hooks ) {
			hooks.beforeEach( function() {
				this.ajaxurl = window.ajaxurl;
				window.ajaxurl = '/wp-admin/admin-ajax.php';
				this.request = jQuery.Deferred();
				this.postStub = sinon.stub( jQuery, 'post' ).returns( this.request );
				this.widget = jQuery(
					'<div id="wp_dashboard_on_this_day">' +
						'<ul id="wp-on-this-day-posts" class="wp-on-this-day-posts">' +
							'<li class="wp-on-this-day-year" data-wp-on-this-day-year="2025">' +
								'<h3>2025</h3>' +
								'<ul class="wp-on-this-day-year-posts"><li>Existing post</li></ul>' +
							'</li>' +
						'</ul>' +
						'<p class="wp-on-this-day-load-more">' +
							'<button class="button-link" data-wp-on-this-day-nonce="nonce" data-wp-on-this-day-offset="10">Show 2 more posts</button>' +
						'</p>' +
					'</div>'
			...
			QUnit.test( 'loads remaining posts and preserves their year groups', function( assert ) {
				var button = this.widget.find( 'button' ),
					posts = this.widget.find( '#wp-on-this-day-posts' );

				button.trigger( 'click' );

				assert.true( button.prop( 'disabled' ), 'The button is disabled while posts load.' );
				assert.deepEqual(
					this.postStub.firstCall.args[1],
					{
						action: 'dashboard-on-this-day-load-more',
						_ajax_nonce: 'nonce',
						offset: '10'
					},
					'The request includes the expected action, nonce, and offset.'
				);
				...
				this.request.resolve(
					{
						...
						data: {
							html:
								'<li class="wp-on-this-day-year" data-wp-on-this-day-year="2025">' +
									'<h3>2025</h3>' +
									'<ul class="wp-on-this-day-year-posts"><li>Another 2025 post</li></ul>' +
								'</li>' +
								'<li class="wp-on-this-day-year" data-wp-on-this-day-year="2024">' +
									'<h3>2024</h3>' +
									'<ul class="wp-on-this-day-year-posts"><li>2024 post</li></ul>' +
								'</li>',
							post_count: 2
						}
					}
				...
				assert.strictEqual( posts.children( '.wp-on-this-day-year' ).length, 2, 'Posts are grouped into two years.' );
				assert.strictEqual( posts.children( '[data-wp-on-this-day-year="2025"]' ).find( 'li' ).length, 2, 'Matching years are merged.' );
				assert.strictEqual( this.widget.find( '.wp-on-this-day-load-more' ).length, 0, 'The button is removed after loading posts.' );
			} );
		} );
	} );
} );
