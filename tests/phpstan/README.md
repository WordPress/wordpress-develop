...
- An object hash that would have to stay open is skipped. PHPStan's object shapes have a fixed member list and no `...`, so sealing one would report every member the hash does not name — which is what a hash on a value from an external API, such as the theme data `WP_Theme_Install_List_Table::single_row()` documents, would produce.
