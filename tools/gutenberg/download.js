...
async function extractVerifiedArchive( archivePath, expectedSha ) {
	const stagingDir = path.join( rootDir, `.gutenberg-download-${ process.pid }` );
	const backupDir = path.join( rootDir, `.gutenberg-backup-${ process.pid }` );
	...
	try {
		...
		if ( fs.existsSync( gutenbergDir ) ) {
			...
			fs.renameSync( gutenbergDir, backupDir );
		}

		try {
			fs.renameSync( stagingDir, gutenbergDir );
		} catch ( error ) {
			if ( fs.existsSync( backupDir ) && ! fs.existsSync( gutenbergDir ) ) {
				try {
					fs.renameSync( backupDir, gutenbergDir );
				} catch ( restoreError ) {
					preserveBackup = true;
					throw new Error(
						`Could not replace Gutenberg directory: ${ /** @type {Error} */ ( error ).message }; ` +
						`could not restore the backup at ${ backupDir }: ${ /** @type {Error} */ ( restoreError ).message }`
					);
				}
			}
