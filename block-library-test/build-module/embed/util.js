 ...
 */
export const getEmbedInfoByProvider = provider => getBlockVariations(DEFAULT_EMBED_BLOCK)?.find(({
  name
}) => name === provider);
 ...
 */
export const findMoreSuitableBlock = url => getBlockVariations(DEFAULT_EMBED_BLOCK)?.find(({
  patterns
 ...
 */
export const getMergedAttributesWithPreview = (currentAttributes, preview, title, isResponsive) => {
  const {
    allowResponsive,
    className
  } = currentAttributes;
