 ...
 */
import { AlignmentControl, BlockControls, InspectorControls, useBlockProps, Warning, HeadingLevelDropdown, store as blockEditorStore } from '@wordpress/block-editor';
...
export default function QueryTitleEdit({
  ...
  if (!SUPPORTED_TYPES.includes(type)) {
    return createElement("div", {
      ...blockProps
    }, createElement(Warning, null, __('Provided type is not supported.')));
  }
  let titleElement;
  if (type === 'archive') {
    let title;
    if (archiveTypeTitle) {
      if (showPrefix) {
        if (archiveNameLabel) {
          ...
          __('%1$s: %2$s'), archiveTypeTitle, archiveNameLabel);
        } else {
          title = sprintf( /* translators: %s: Archive type title e.g: "Category", "Tag"... */
          __('%s: Name'), archiveTypeTitle);
        }
  ...
  if (type === 'search') {
    titleElement = createElement(Fragment, null, createElement(InspectorControls, null, createElement(PanelBody, {
      ...
      ...blockProps
    }, showSearchTerm ? __('Search results for: “search term”') : __('Search results')));
  }
