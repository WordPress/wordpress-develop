 ...
 *
 * For input fields where a user can be selected by searching their name, login,
 * or email, this script shows an autocompletion menu. On multisite, only users
 * in the currently active site are shown for `search` and `site_search` types.
 *
 * @since 3.4.0
...
(function( $ ) {
	...
	$( function() {
		 ...
		 * - data-autocomplete-type (add, search, site_search)
		 *   The action to perform: search the network to add a user, search a
		 *   site's members, or search the current site's users. Default: add
		 * - data-autocomplete-field (user_login, user_email, user_id)
		 *   The field returned as the value for the suggestion.
		 ...
		 *   Default: empty (server returns the display name).
		 *
		...
		$( '.wp-suggest-user' ).each( function() {
			var $this             = $( this ),
				...
				// True when using the user_id field with a sibling helper input.
