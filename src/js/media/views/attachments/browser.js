...
AttachmentsBrowser = View.extend(/** @lends wp.media.view.AttachmentsBrowser.prototype */{
	...
	createLoadMoreView: function() {
		...
		this.loadMoreJumpToFirst = new wp.media.view.Button( {
			text: __( 'Jump to first new item' ),
			className: 'load-more-jump visually-hidden',
			size: '',
			click: function() {
	...
	updateLoadMoreView: _.debounce( function() {
		...
		this.loadMoreJumpToFirst.$el.addClass( 'visually-hidden' );
		...
		// If there's a media item to move focus to, make the "Jump to" button available on focus.
		if ( this.firstAddedMediaItem.length ) {
			this.loadMoreJumpToFirst.$el.removeClass( 'visually-hidden' );
		}
	}, 10 ),
	...
	loadMoreAttachments: function() {
		 ...
		 * amount of items. Thus the length is equivaent to the position of the
		 * first added item.
		 */
