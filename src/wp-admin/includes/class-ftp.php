...
// <!-- --------------------------------------------------------------------------------------- -->
	...
	function connect($server=NULL) {
		...
		$this->_ready=true;
		$syst=$this->systype();
		if(!$syst) $this->SendMSG("Can not detect remote OS");
		else {
		...
		if(!$this->features()) $this->SendMSG("Can not get features list. All supported - disabled");
	...
	function restore($from) {
		...
		if($this->_curtype!=FTP_BINARY) {
			$this->PushError("restore", "can not restore in ASCII mode");
			return FALSE;
		}
	...
	function get($remotefile, $localfile=NULL, $rest=0) {
		...
		if (!$fp) {
			$this->PushError("get","can not open local file", "Cannot create \"".$localfile."\"");
			return FALSE;
		}
		if($this->_can_restore and $rest!=0) fseek($fp, $rest);
		$pi=pathinfo($remotefile);
	...
	function put($localfile, $remotefile=NULL, $rest=0) {
		...
		if (!file_exists($localfile)) {
			$this->PushError("put","can not open local file", "No such file or directory \"".$localfile."\"");
			return FALSE;
		}
		$fp = @fopen($localfile, "r");

		if (!$fp) {
			$this->PushError("put","can not open local file", "Cannot read file \"".$localfile."\"");
			return FALSE;
		}
		if($this->_can_restore and $rest!=0) fseek($fp, $rest);
	...
	function mput($local=".", $remote=NULL, $continious=false) {
		...
		if(!@file_exists($local)) {
			$this->PushError("mput","can not open local folder", "Cannot stat folder \"".$local."\"");
			return FALSE;
		}
		...
		if($handle = opendir($local)) {
			...
			$this->PushError("mput","can not open local folder", "Cannot read folder \"".$local."\"");
			return FALSE;
		}
	...
	function mget($remote, $local=".", $continious=false) {
		...
		if($list===false) {
			$this->PushError("mget","can not read remote folder list", "Can not read remote folder \"".$remote."\" contents");
			return FALSE;
		}
		if(empty($list)) return true;
		if(!@file_exists($local)) {
			if(!@mkdir($local)) {
				$this->PushError("mget","can not create local folder", "Cannot create folder \"".$local."\"");
				return FALSE;
			}
		}
		foreach($list as $k=>$v) {
		...
		foreach($list as $el) {
			if($el["type"]=="d") {
				if(!$this->mget($remote."/".$el["name"], $local."/".$el["name"], $continious)) {
					$this->PushError("mget", "can not copy folder", "Can not copy remote folder \"".$remote."/".$el["name"]."\" to local \"".$local."/".$el["name"]."\"");
					$ret=false;
					if(!$continious) break;
				}
			} else {
				if(!$this->get($remote."/".$el["name"], $local."/".$el["name"])) {
					$this->PushError("mget", "can not copy file", "Can not copy remote file \"".$remote."/".$el["name"]."\" to local \"".$local."/".$el["name"]."\"");
					$ret=false;
					if(!$continious) break;
				}
			}
	...
	function mdel($remote, $continious=false) {
		$list=$this->rawlist($remote, "-la");
		if($list===false) {
			$this->PushError("mdel","can not read remote folder list", "Can not read remote folder \"".$remote."\" contents");
			return false;
		}
		...
		foreach($list as $el) {
			...
			if($el["type"]=="d") {
				...
				if (!$this->delete($remote."/".$el["name"])) {
					$this->PushError("mdel", "can not delete file", "Can not delete remote file \"".$remote."/".$el["name"]."\"");
					$ret=false;
					if(!$continious) break;
				}
			}
		...
		if(!$this->rmdir($remote)) {
			$this->PushError("mdel", "can not delete folder", "Can not delete remote folder \"".$remote."/".$el["name"]."\"");
			$ret=false;
		}
		return $ret;
	}
	...
	function dirlist($remote) {
		$list=$this->rawlist($remote, "-la");
		if($list===false) {
			$this->PushError("dirlist","can not read remote folder list", "Can not read remote folder \"".$remote."\" contents");
			return false;
		}
