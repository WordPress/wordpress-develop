...
function edit_post( $post_data = null ) {
	...
	$post_id_from_data = (int) $post_data['post_ID'];
	$post    = get_post( $post_id_from_data );
	...
	$ptype = get_post_type_object( $post_data['post_type'] );
	if ( ! current_user_can( 'edit_post', $post_id_from_data ) ) {
		if ( 'page' === $post_data['post_type'] ) {
	...
	if ( post_type_supports( $ptype->name, 'revisions' ) ) {
		...
		if ( $revisions && _wp_get_post_revision_version( $revision ) < 1 ) {
			_wp_upgrade_revisions_of_post( $post, wp_get_post_revisions( $post_id_from_data ) );
		}
	}
	...
	if ( isset( $post_data['post_format'] ) ) {
		set_post_format( $post_id_from_data, $post_data['post_format'] );
	}
	...
	foreach ( $format_meta_urls as $format_meta_url ) {
		...
		if ( isset( $post_data[ $keyed ] ) ) {
			update_post_meta( $post_id_from_data, $keyed, wp_slash( sanitize_url( wp_unslash( $post_data[ $keyed ] ) ) ) );
		}
	}
	...
	foreach ( $format_keys as $key ) {
		...
		if ( isset( $post_data[ $keyed ] ) ) {
			if ( current_user_can( 'unfiltered_html' ) ) {
				update_post_meta( $post_id_from_data, $keyed, $post_data[ $keyed ] );
			} else {
				update_post_meta( $post_id_from_data, $keyed, wp_filter_post_kses( $post_data[ $keyed ] ) );
			}
		}
	}

	if ( 'attachment' === $post_data['post_type'] && preg_match( '#^(audio|video)/#', $post_data['post_mime_type'] ) ) {
		$id3data = wp_get_attachment_metadata( $post_id_from_data );
		if ( ! is_array( $id3data ) ) {
			$id3data = array();
		}
		...
		wp_update_attachment_metadata( $post_id_from_data, $id3data );
	}
	...
	if ( isset( $post_data['meta'] ) && $post_data['meta'] ) {
		foreach ( $post_data['meta'] as $key => $value ) {
			...
			if ( $meta->post_id != $post_id_from_data ) {
				continue;
			}
			if ( is_protected_meta( $meta->meta_key, 'post' ) || ! current_user_can( 'edit_post_meta', $post_id_from_data, $meta->meta_key ) ) {
				continue;
			}
			if ( is_protected_meta( $value['key'], 'post' ) || ! current_user_can( 'edit_post_meta', $post_id_from_data, $value['key'] ) ) {
				continue;
			}
	...
	if ( isset( $post_data['deletemeta'] ) && $post_data['deletemeta'] ) {
		foreach ( $post_data['deletemeta'] as $key => $value ) {
			...
			if ( is_protected_meta( $meta->meta_key, 'post' ) || ! current_user_can( 'delete_post_meta', $post_id_from_data, $meta->meta_key ) ) {
				continue;
			}
			delete_meta( $key );
		}
	...
	if ( 'attachment' === $post_data['post_type'] ) {
		if ( isset( $post_data['_wp_attachment_image_alt'] ) ) {
			...
			if ( get_post_meta( $post_id_from_data, '_wp_attachment_image_alt', true ) !== $image_alt ) {
				$image_alt = wp_strip_all_tags( $image_alt, true );

				// update_post_meta() expects slashed.
				update_post_meta( $post_id_from_data, '_wp_attachment_image_alt', wp_slash( $image_alt ) );
			}
		}

		$attachment_data = isset( $post_data['attachments'][ $post_id_from_data ] ) ? $post_data['attachments'][ $post_id_from_data ] : array();
	...
	add_meta( $post_id_from_data );

	update_post_meta( $post_id_from_data, '_edit_last', get_current_user_id() );
	...
	_fix_attachment_links( $post_id_from_data );

	wp_set_post_lock( $post_id_from_data );

	if ( current_user_can( $ptype->cap->edit_others_posts ) && current_user_can( $ptype->cap->publish_posts ) ) {
		if ( ! empty( $post_data['sticky'] ) ) {
			stick_post( $post_id_from_data );
		} else {
			unstick_post( $post_id_from_data );
		}
	}

	return $post_id_from_data;
}
...
function bulk_edit_posts( $post_data = null ) {
	...
	foreach ( $post_ids as $post_id_from_data ) {
		// Start with fresh post data with each iteration.
		$post_data = $shared_post_data;

		$post_type_object = get_post_type_object( get_post_type( $post_id_from_data ) );

		if ( ! isset( $post_type_object )
			|| ( isset( $children ) && in_array( $post_id_from_data, $children, true ) )
			|| ! current_user_can( 'edit_post', $post_id_from_data )
		) {
			$skipped[] = $post_id_from_data;
			continue;
		}

		if ( wp_check_post_lock( $post_id_from_data ) ) {
			$locked[] = $post_id_from_data;
			continue;
		}
		...
		foreach ( $tax_names as $tax_name ) {
			...
			if ( $taxonomy_obj->hierarchical ) {
				$current_terms = (array) wp_get_object_terms( $post_id_from_data, $tax_name, array( 'fields' => 'ids' ) );
			} else {
				$current_terms = (array) wp_get_object_terms( $post_id_from_data, $tax_name, array( 'fields' => 'names' ) );
			}
		...
		if ( isset( $new_cats ) && in_array( 'category', $tax_names, true ) ) {
			$cats                       = (array) wp_get_post_categories( $post_id_from_data );
			$post_data['post_category'] = array_unique( array_merge( $cats, $new_cats ) );
			unset( $post_data['tax_input']['category'] );
		}

		$post_data['post_ID']        = $post_id_from_data;
		$post_data['post_type']      = $post->post_type;
		$post_data['post_mime_type'] = $post->post_mime_type;
		...
		if ( isset( $shared_post_data['post_format'] ) ) {
			set_post_format( $post_id_from_data, $shared_post_data['post_format'] );
		}
...
function wp_write_post() {
	...
	// Create the post.
	$post_id_from_data = wp_insert_post( $translated );
	if ( is_wp_error( $post_id_from_data ) ) {
		return $post_id_from_data;
	}

	if ( empty( $post_id_from_data ) ) {
		return 0;
	}

	add_meta( $post_id_from_data );

	add_post_meta( $post_id_from_data, '_edit_last', $GLOBALS['current_user']->ID );
 ...
 * @since 1.2.0
 *
 * @param int $post_id_from_data
 * @return int|bool
 */
function add_meta( $post_id_from_data ) {
	$post_id_from_data = (int) $post_id_from_data;
	...
	if ( ( ( '#NONE#' !== $metakeyselect ) && ! empty( $metakeyselect ) ) || ! empty( $metakeyinput ) ) {
		...
		if ( is_protected_meta( $metakey, 'post' ) || ! current_user_can( 'add_post_meta', $post_id_from_data, $metakey ) ) {
			return false;
		}

		$metakey = wp_slash( $metakey );

		return add_post_meta( $post_id_from_data, $metakey, $metavalue );
	}
...
function post_preview() {
	...
	$post_id_from_data     = (int) $_POST['post_ID'];
	$_POST['ID'] = $post_id_from_data;
