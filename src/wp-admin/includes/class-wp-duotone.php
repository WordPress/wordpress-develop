...
class WP_Duotone {
	...
	public static function set_global_style_block_names() {
		...
		foreach ( $block_nodes as $block_node ) {
			...
			$slug = self::wp_get_slug_from_attr( $duotone_attr );
	 ...
	 */
	private static function wp_get_slug_from_attr( $duotone_attr ) {
	...
	private static function get_css_custom_property_declaration( $filter_data ) {
		$declaration_value                = wp_get_duotone_filter_property( $filter_data );
	...
	public static function output_footer_assets() {
		foreach ( self::$output as $filter_data ) {
			...
			$filter_svg = wp_get_duotone_filter_svg( $filter_data );
	...
	public static function render_duotone_support( $block_content, $block ) {
		...
		$filter_id = wp_get_duotone_filter_id( array( 'slug' => $slug ) );
