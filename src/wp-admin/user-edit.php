			...
			<form id="your-profile" action="<?php echo esc_url( self_admin_url( IS_PROFILE_PAGE ? 'profile.php' : 'user-edit.php' ) ); ?>" method="post" novalidate="novalidate"
				...
				<table class="form-table" role="presentation">
					<tr class="user-email-wrap">
						...
						<td>
							<input type="email" name="email" id="email" aria-describedby="email-description" value="<?php echo esc_attr( $profileuser->user_email ); ?>" class="regular-text ltr" />
							<?php if ( $profileuser->ID == $current_user->ID ) : ?>
							...
							<?php if ( $new_email && $new_email['newemail'] != $current_user->user_email && $profileuser->ID == $current_user->ID ) : ?>
					...
					<?php foreach ( wp_get_user_contact_methods( $profileuser ) as $name => $desc ) : ?>
				...
				<table class="form-table" role="presentation">
					 ...
					 * @param WP_User $profileuser The current WP_User object.
					 */
					$display_additional_caps = apply_filters( 'additional_capabilities_display', true, $profileuser );
					?>

				<?php if ( count( $profileuser->caps ) > count( $profileuser->roles ) && ( true === $display_additional_caps ) ) : ?>
