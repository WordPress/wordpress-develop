	...
	private static function register_find_posts(): void {
		wp_register_ability(
			...
			array(
				...
				'description'         => sprintf(
					/* translators: %s: comma-separated list of available post types */
					__( 'Search and filter WordPress posts. Supports filtering by post type, status, author, taxonomy terms, meta fields, and date ranges. Available post types: %s.' ),
					self::$available_post_types_desc
				),
				'input_schema'        => array(
					'type'       => 'object',
					'properties' => array(
						'post_type'          => array(
							'type'        => 'string',
							'description' => __( 'Post type to search. Defaults to "post".' ),
							'default'     => 'post',
						),
						'post_status'        => array(
							'type'        => 'array',
							'description' => __( 'Post statuses to include.' ),
							'items'       => array( 'type' => 'string' ),
							'default'     => array( 'publish' ),
						),
						'search'             => array(
							'type'        => 'string',
							'description' => __( 'Search query to filter posts by title and content.' ),
						),
						'author'             => array(
							'type'        => 'integer',
						...
						'limit'              => array(
							'type'        => 'integer',
							'description' => __( 'Maximum number of posts to return (max 100).' ),
							'default'     => 10,
							'minimum'     => 1,
							'maximum'     => 100,
						),
						...
						'order'              => array(
							'type'        => 'string',
							'description' => __( 'Sort order.' ),
							'enum'        => array( 'ASC', 'DESC' ),
							'default'     => 'DESC',
						),
						'tax_query'          => array(
							'type'        => 'object',
							'description' => __( 'Taxonomy query with nested queries and relations.' ),
							'properties'  => array(
								'relation' => array(
									'type'        => 'string',
									'description' => __( 'Logical relation between queries.' ),
									'enum'        => array( 'AND', 'OR' ),
								),
								'queries'  => array(
									'type'        => 'array',
									'description' => __( 'Array of taxonomy clauses or nested groups.' ),
									'items'       => array(
										'oneOf' => array(
											array(
												'type'     => 'object',
												...
												'properties' => array(
													'taxonomy' => array(
														'type'        => 'string',
														'description' => __( 'Taxonomy name to query.' ),
													),
													'terms'    => array(
														'type'        => 'array',
														'description' => __( 'Term IDs, slugs, or names to match.' ),
														'items'       => array( 'type' => array( 'integer', 'string' ) ),
													),
													'field'    => array(
														'type'        => 'string',
														'description' => __( 'Field to match terms against. Defaults to "term_id".' ),
														'enum'        => array( 'term_id', 'slug', 'name', 'term_taxonomy_id' ),
													),
													'operator' => array(
														'type'        => 'string',
														'description' => __( 'Operator for comparison. Defaults to "IN".' ),
														'enum'        => array( 'IN', 'NOT IN', 'AND', 'EXISTS', 'NOT EXISTS' ),
													),
													'include_children' => array(
														'type'        => 'boolean',
														'description' => __( 'Include child terms. Defaults to true.' ),
													),
												),
											),
											array(
												...
												'properties' => array(
													'relation' => array(
														'type'        => 'string',
														'description' => __( 'Relation for nested group. Defaults to "AND".' ),
														'enum'        => array( 'AND', 'OR' ),
													),
													'queries'  => array(
														'type'        => 'array',
														'description' => __( 'Nested taxonomy clauses.' ),
													),
												),
											),
										),
						...
						'meta_query'         => array(
							'type'        => 'object',
							'description' => __( 'Meta query with nested queries and relations.' ),
							'properties'  => array(
								'relation' => array(
									'type'        => 'string',
									'description' => __( 'Logical relation between queries.' ),
								...
								'queries'  => array(
									'type'        => 'array',
									'description' => __( 'Array of query clauses or nested groups.' ),
									'items'       => array(
										'oneOf' => array(
											array(
												'type'     => 'object',
												...
												'properties' => array(
													...
													'value' => array(
														...
														'description' => __( 'Meta value. Not required for EXISTS/NOT EXISTS.' ),
													),
													'compare' => array(
														'type'        => 'string',
														'description' => __( 'Comparison operator. Defaults to "=".' ),
														'enum'        => array( '=', '!=', '>', '>=', '<', '<=', 'LIKE', 'NOT LIKE', 'IN', 'NOT IN', 'BETWEEN', 'NOT BETWEEN', 'EXISTS', 'NOT EXISTS', 'REGEXP', 'NOT REGEXP', 'RLIKE' ),
													),
													'type' => array(
														'type'        => 'string',
														'description' => __( 'Value type for casting.' ),
						...
						'date_query'         => array(
							'type'        => 'object',
							'description' => __( 'Date query with nested queries and relations.' ),
							'properties'  => array(
								'relation' => array(
									'type'        => 'string',
									'description' => __( 'Logical relation between queries.' ),
								...
								'column'   => array(
									'type'        => 'string',
									'description' => __( 'Column to query against. Defaults to "post_date".' ),
									'enum'        => array( 'post_date', 'post_date_gmt', 'post_modified', 'post_modified_gmt' ),
								),
								'queries'  => array(
									'type'        => 'array',
									'description' => __( 'Array of date clauses or nested groups.' ),
									'items'       => array(
										'oneOf' => array(
											array(
												'type' => 'object',
												...
												'properties' => array(
													'year' => array(
														'type'        => 'integer',
														'description' => __( '4-digit year (e.g., 2023).' ),
													),
													'month' => array(
														'type'        => 'integer',
													...
													'week' => array(
														'type'        => 'integer',
														'description' => __( 'Week of year (0-53).' ),
													),
													'day'  => array(
														'type'        => 'integer',
													...
													'dayofweek' => array(
														'type'        => 'integer',
														'description' => __( 'Day of week (1-7, Sunday=1).' ),
													),
													'dayofweek_iso' => array(
														'type'        => 'integer',
														'description' => __( 'ISO day of week (1-7, Monday=1).' ),
													),
													'dayofyear' => array(
														'type'        => 'integer',
													...
													'after' => array(
														...
														'description' => __( 'Date to retrieve posts after. String or object with year/month/day.' ),
													),
													'before' => array(
														'oneOf'       => array(
															array( 'type' => 'string' ),
														...
														'description' => __( 'Date to retrieve posts before. String or object with year/month/day.' ),
													),
													'inclusive' => array(
														'type'        => 'boolean',
														'description' => __( 'Include posts from the boundary date.' ),
													),
													'compare' => array(
														'type'        => 'string',
													...
													'column' => array(
														'type'        => 'string',
														'description' => __( 'Column to query against for this clause.' ),
														'enum'        => array( 'post_date', 'post_date_gmt', 'post_modified', 'post_modified_gmt' ),
													),
												),
											),
											array(
												...
												'properties' => array(
													...
													'queries'  => array(
														'type'        => 'array',
														'description' => __( 'Nested date clauses.' ),
													),
												),
											),
										),
						...
						'include_taxonomies' => array(
							'type'        => 'boolean',
							'description' => __( 'Include taxonomy terms for each post.' ),
							'default'     => false,
						),
						'include_meta'       => array(
							'type'        => 'boolean',
							'description' => __( 'Include meta fields for each post.' ),
							'default'     => false,
						),
					),
				),
	...
	private static function register_update_post(): void {
		wp_register_ability(
			...
			array(
				...
				'description'         => sprintf(
					/* translators: %s: comma-separated list of available post types */
					__( 'Update an existing WordPress post. Supports partial updates - only provided fields will be modified. Available post types: %s.' ),
					self::$available_post_types_desc
				),
				'input_schema'        => array(
					'type'       => 'object',
