 ...
 * @access private
 *
 * @see _wp_resolve_signup_key()
 *
 * @param string $user_email The user's email address, embedded in the payload.
 * @return array {
 *     @type string $type    Key type. Always 'hmac_sha256'.
 *     @type string $key     The random component embedded in the payload.
 ...
 *   `email:timestamp:random`, whose HMAC-SHA256 is stored in the database.
 * - `md5` (deprecated in 7.1.0): a plain 16-character lowercase hex string
 *   generated by `substr( md5( … ), 0, 16 )` and stored directly in the database.
 *
 * @since 7.1.0
 * @access private
 *
 * @see _wp_generate_signup_key()
 *
 * @param string $key The activation key from the email link.
 * @return array|null {
 *     Parsed key data, or null if the key format is not recognized.
 *
 *     @type string      $format    'hmac_sha256' for keys introduced in 7.1.0, 'md5' for pre-7.1.0 keys.
 *     @type string      $hash      The value to look up in the `activation_key` column.
 *     @type string|null $email     User email embedded in the key ('hmac_sha256' format only).
 *     @type int|null    $timestamp Key issuance timestamp embedded in the key ('hmac_sha256' format only).
 * }
 */
function _wp_resolve_signup_key( $key ) {
	...
	if ( 3 === count( $parts ) && is_email( $parts[0] ) && ctype_digit( $parts[1] ) ) {
		// hmac_sha256 format (since 7.1.0): email:timestamp:random.
	...
	if ( 1 === preg_match( '~^[0-9a-f]{16}$~', $key ) ) {
		// md5 format (deprecated in 7.1.0): plain 16-char hex, stored directly in the DB.
		return array(
			'format'    => 'md5',
			'hash'      => $key,
			'email'     => null,
...
function wpmu_signup_blog( $domain, $path, $title, $user, $user_email, $meta = array() ) {
	 ...
	 *
	 * @since 4.4.0
	 * @since 7.1.0 The `$key` parameter format changed. Previously a 16-character MD5 hex
	 *              string stored directly in the database; now a URL-safe base64-encoded
	 *              HMAC payload (`email:timestamp:random`) to embed in activation links.
	 *
	 * @param string $domain     The requested domain.
	 * @param string $path       The requested path.
	 * @param string $title      The requested site title.
	 * @param string $user       The user's requested login name.
	 * @param string $user_email The user's email address.
	 * @param string $key        The activation key to embed in the activation URL.
...
function wpmu_activate_signup(
	...
	/*
	 * Both hmac_sha256 and md5 formats are looked up by the resolved hash value.
	 * The HMAC cryptographically binds the email to the hash, so querying by hash alone
	 * is sufficient and avoids mixing a known public value (email) with the secret lookup.
	 */
	...
	/*
	 * For hmac_sha256 keys, use the timestamp from the cryptographically bound payload —
	 * it cannot be extended by modifying the database. md5 keys have no embedded timestamp,
	 * so fall back to the `registered` column.
	 */
	$issued_at = 'hmac_sha256' === $resolved['format']
		? $resolved['timestamp']
