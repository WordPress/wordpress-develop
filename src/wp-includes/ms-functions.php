 ...
 *
 * - `hmac_sha256` (introduced in 6.9.0): a URL-safe base64-encoded triplet
 *   `email:timestamp:random`, whose HMAC-SHA256 is stored in the database.
 * - `md5` (deprecated in 6.9.0): a plain 16-character lowercase hex string
 ...
 *
 *     @type string      $format    'hmac_sha256' for keys introduced in 6.9.0, 'md5' for pre-6.9.0 keys.
...
function _wp_resolve_signup_key( $key ) {
	...
	if ( 3 === count( $parts ) && is_email( $parts[0] ) && ctype_digit( $parts[1] ) ) {
		// hmac_sha256 format (since 6.9.0): email:timestamp:random.
	...
	if ( 1 === preg_match( '~^[0-9a-f]{16}$~', $key ) ) {
		// md5 format (deprecated in 6.9.0): plain 16-char hex, stored directly in the DB.
		return array(
			'format'    => 'md5',
			'hash'      => $key,
			'email'     => null,
...
function wpmu_signup_blog( $domain, $path, $title, $user, $user_email, $meta = array() ) {
	 ...
	 *
	 * @since 4.4.0
	 * @since 6.9.0 The `$key` parameter format changed. Previously a 16-character MD5 hex
...
function wpmu_activate_signup(
	...
	if ( 'hmac_sha256' === $resolved['format'] ) {
		/*
		...
		// md5 format (pre-6.9.0): plain hex key stored directly in the DB.
		$signup = $wpdb->get_row(
	...
	// For hmac_sha256 keys, use the cryptographically bound timestamp from the payload.
	// For md5 (legacy) keys, fall back to the registered column in the database.
