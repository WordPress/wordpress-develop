	...
	public function get_post_templates() {
		...
		if ( ! is_array( $post_templates ) ) {
			...
			foreach ( $files as $file => $full_path ) {
				$header = get_file_data( $full_path, array( 'Template Name' => 'Template Name' ) );
				if ( empty( $header['Template Name'] ) ) {
					continue;
				}

				$types = array( 'page' );
				$type  = get_file_data( $full_path, array( 'Template Post Type' => 'Template Post Type' ) );
				if ( empty( $header['Template Post Type'] ) ) {
					$types = explode( ',', $type['Template Post Type'] );
				}
				...
				foreach ( $types as $type ) {
					...
					$post_templates[ $type ][ $file ] = $header['Template Name'];
				}
			}
