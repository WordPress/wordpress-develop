...
function _register_theme_block_patterns() {
	...
	foreach ( $themes as $theme ) {
		...
		$pattern_datas = false;
		if ( $can_use_cached ) {
			$pattern_datas = get_transient( $transient_name );
			if ( is_array( $pattern_datas ) && $pattern_datas['version'] !== $version ) {
				$pattern_datas = false;
			}
		}
		...
		if ( false === $pattern_datas ) {
			...
			if ( $can_use_cached ) {
				set_transient( $transient_name, $pattern_datas );
			}
		}

		foreach ( $pattern_datas['patterns'] as $file => $pattern_data ) {
