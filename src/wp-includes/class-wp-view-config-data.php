	...
	private function merge_properties( $current, $incoming, $replace_lists, $method ) {
		...
		if ( array_is_list( $incoming ) ) {
			...
			if ( is_array( $current ) && ! array_is_list( $current ) && array() !== $current ) {
				_doing_it_wrong(
					esc_html( $method ),
					esc_html__( 'A list cannot be merged into an associative value; name the keys to change instead.' ),
					'7.1.0'
				);
				return $current;
			}
		...
		if ( is_array( $current ) && array_is_list( $current ) && array() !== $current ) {
			_doing_it_wrong(
				esc_html( $method ),
				esc_html__( 'An associative array cannot be merged into a list; address list members by their identity instead.' ),
				'7.1.0'
			);
			return $current;
		}
