...
function wp_is_block_editor() {
	...
	return ! empty( $screen ) &&
		(
			...
			'appearance_page_block-widgets' === $screen->id ||
			( function_exists( 'wp_is_edit_site_page' ) && wp_is_edit_site_page( $screen->id ) )
		);
}
 ...
 * @since 5.6.0
 */
function wp_block_editor_admin_print_styles() {
...
add_action( 'admin_print_styles', 'wp_block_editor_admin_print_styles' );
 ...
 * @since 5.6.0
 */
function wp_block_editor_admin_print_scripts() {
	if ( wp_is_block_editor() ) {
...
add_action( 'admin_print_scripts', 'wp_block_editor_admin_print_scripts' );
 ...
 * @since 5.6.0
 */
function wp_block_editor_admin_print_footer_scripts() {
	if ( wp_is_block_editor() ) {
		/** This action is documented in wp-admin/admin-footer.php */
		// phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
...
add_action( 'admin_print_footer_scripts', 'wp_block_editor_admin_print_footer_scripts' );
 ...
 * @since 5.6.0
 */
function wp_block_editor_admin_footer() {
	if ( wp_is_block_editor() ) {
		/** This action is documented in wp-admin/admin-footer.php */
		// phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
...
add_action( 'admin_footer', 'wp_block_editor_admin_footer' );
 ...
 * @since 5.6.0
 */
function wp_print_save_widgets_nonce() {
...
add_action( 'admin_footer-widgets.php', 'wp_print_save_widgets_nonce' );
 ...
 */
function wp_get_legacy_widget_settings() {
	$settings = array();
 ...
 */
function wp_legacy_widget_settings( $settings ) {
	return array_merge( $settings, wp_get_legacy_widget_settings() );
}
add_filter( 'block_editor_settings', 'wp_legacy_widget_settings' );
...
add_action( 'enqueue_block_editor_assets', 'wp_enqueue_widget_scripts' );
 ...
 */
function wp_override_sidebar_params_for_block_widget( $arg ) {
...
function wp_register_widgets() {
	...
	if ( 'widgets.php' === $pagenow ) {
		add_filter(
			...
			'wp_override_sidebar_params_for_block_widget'
		);
	}
}
