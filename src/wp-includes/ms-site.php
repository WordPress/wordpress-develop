...
function wp_insert_site( array $data ) {
	...
	if ( has_action( 'wpmu_new_blog' ) ) {
		...
		// The `$site_data_allowedlist` matches the one used in `wpmu_create_blog()`.
		$site_data_allowedlist = array( 'public', 'archived', 'mature', 'spam', 'deleted', 'lang_id' );
		$meta                = array_merge( array_intersect_key( $data, array_flip( $site_data_allowedlist ) ), $meta );
...
function wp_prepare_site_data( $data, $defaults, $old_site = null ) {
	...
	$allowedlist = array( 'domain', 'path', 'network_id', 'registered', 'last_updated', 'public', 'archived', 'mature', 'spam', 'deleted', 'lang_id' );
	$data      = array_intersect_key( wp_parse_args( $data, $defaults ), array_flip( $allowedlist ) );
