...
function current_theme_supports( $feature, ...$args ) {
	...
	// If args passed, extra checks need to be performed.
	if ( $retval && $args ) {
		switch ( $feature ) {
			case 'post-thumbnails':
				/*
				 ...
				 */
				if ( true !== $_wp_theme_features[ $feature ] ) {  // Not registered for all types.
					$content_type = $args[0];
					$retval       = in_array( $content_type, $_wp_theme_features[ $feature ][0], true );
				}
				break;
			...
			case 'post-formats':
				 ...
				 */
				$type   = $args[0];
				$retval = in_array( $type, $_wp_theme_features[ $feature ][0], true );
				break;
			...
			case 'custom-background':
				...
				$retval = ! empty( $_wp_theme_features[ $feature ][0][ $args[0] ] );
				break;
		}
	}
	 ...
	 */
	return apply_filters( "current_theme_supports-{$feature}", $retval, $args, $_wp_theme_features[ $feature ] ?? null ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
