 ...
 *
 * Post statuses registered as private, protected or internal are never
 * considered viewable. Public statuses are viewable.
 *
 * @since 5.6.0
 *
...
function is_post_viewable( $post ) {
	$post = get_post( $post );
	if ( ! $post ) {
		return false;
	}
	...
	if ( is_post_status_viewable( $post_status ) ) {
		return true;
	}

	// Now check if the current user can view the post status.
