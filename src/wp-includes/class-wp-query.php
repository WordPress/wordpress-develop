		...
		if ( $skip_post_status ) {
			...
			if ( empty( $q['post_status'] ) ) {
				...
				foreach ( $public_statuses as $public_status ) {
					$r_status[] = "{$wpdb->posts}.post_status = '$public_status'";
				}
				...
				if ( $this->is_admin ) {
					...
					foreach ( $admin_all_statuses as $admin_all_status ) {
						$r_status[] = "{$wpdb->posts}.post_status = '$admin_all_status'";
					}
				}
				...
				foreach ( $private_statuses as $private_status ) {
					$p_status[] = "{$wpdb->posts}.post_status = '$private_status'";
				}
			...
			$found_at_least_one_valid_status_rule = ! empty( $r_status ) || ! empty( $p_status ) || ! empty( $e_status );

			if ( $found_at_least_one_valid_status_rule && ! empty( $queried_post_types ) ) {
				...
				foreach ( $queried_post_types as $queried_post_type ) {
					...
					if ( is_user_logged_in() && ! empty( $p_status ) ) {
					...
					if ( ! empty( $q_status ) && $post_status_join ) {
