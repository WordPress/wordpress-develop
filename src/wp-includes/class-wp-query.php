...
class WP_Query {
	...
	public function get_posts() {
		...
		if ( $this->is_comment_feed && ! $this->is_singular ) {
			...
			$comment_ids = wp_cache_get( $cache_key, 'queries' );
			if ( false === $comment_ids ) {
				$comment_ids = $wpdb->get_col( $comments_request );
				wp_cache_add( $cache_key, $comment_ids, 'queries' );
			}
		...
		if ( $q['cache_results'] && $id_query_is_cacheable ) {
			...
			$cache_found = false;
			if ( null === $this->posts ) {
				$cached_results = wp_cache_get( $cache_key, 'queries', false, $cache_found );
		...
		if ( 'ids' === $q['fields'] ) {
			...
			if ( $q['cache_results'] && $id_query_is_cacheable ) {
				...
				wp_cache_set( $cache_key, $cache_value, 'queries' );
			}
		...
		if ( ! empty( $this->posts ) && $this->is_comment_feed && $this->is_singular ) {
			...
			$comment_ids       = wp_cache_get( $comment_cache_key, 'queries' );
			if ( false === $comment_ids ) {
				$comment_ids = $wpdb->get_col( $comments_request );
				wp_cache_add( $comment_cache_key, $comment_ids, 'queries' );
			}
			_prime_comment_caches( $comment_ids, false );
