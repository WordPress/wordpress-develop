...
function get_oembed_response_data_rich( $data, $post, $width, $height ) {
	...
	if ( $thumbnail_id ) {
		$image_src = wp_get_attachment_image_src( $thumbnail_id, array( $width, 0 ) );
		if ( is_array( $image_src ) ) {
			$data['thumbnail_url'] = (string) $image_src[0];
			$data['thumbnail_width'] = (int) $image_src[1];
			$data['thumbnail_height'] = (int) $image_src[2];
		}
	}
