...
class WP_Duotone {
	...
	public static function add_editor_settings( $settings ) {
		...
		foreach ( self::$global_styles_presets as $filter_data ) {
			$duotone_svgs .= get_duotone_filter_svg( $filter_data );
		...
		$settings['styles'][] = array(
			...
			// These styles are no longer generated by global styles, so this must be false or they will be stripped out in get_block_editor_settings.
			'isGlobalStyles' => false,
		);
