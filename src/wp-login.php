...
if ( ! in_array( $action, array( 'postpass', 'logout', 'lostpassword', 'retrievepassword', 'resetpass', 'rp', 'register', 'login', 'confirmaction', 'begun_recovery_mode' ), true ) && false === has_filter( 'login_form_' . $action ) ) {
	$action = 'login';
}
...
switch ( $action ) {
	...
	default:
		...
		if ( $interim_login ) {
			...
			} elseif ( 'begun_recovery_mode' === $action ) {
				$errors->add( 'begun_recovery_mode', __( 'Recovery Mode Initialized. Please login to continue.' ), 'message' );
			}
		}
