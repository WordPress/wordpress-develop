...
module.exports = function(grunt) {
	...
	/*
	 * Replaces grunt-contrib-imagemin, last released in 2020, which pins
	 * versions of its image binaries that were published in 2018.
	 */
	grunt.registerMultiTask( 'imagemin', 'Losslessly optimizes PNG, JPEG and GIF images.', function() {
		const done = this.async();
		...
		async function optimize( src, contents ) {
			...
			const result = spawn( binary, [ '--optimize=3', '--interlace', '--output', '-', src ], { maxBuffer: Infinity } );
		...
		async function work() {
			...
			while ( ( file = files.shift() ) ) {
				...
				// Images that are already optimal can come back slightly larger.
				const output = result.length < contents.length ? result : contents;

				if ( output !== contents ) {
					optimized++;
					savedBytes += contents.length - output.length;
					grunt.verbose.writeln( src + ' (saved ' + ( contents.length - output.length ) + ' bytes)' );
				} else {
		...
		async function run() {
			...
			const workers = Math.min( Math.max( require( 'os' ).cpus().length, 1 ), total );
